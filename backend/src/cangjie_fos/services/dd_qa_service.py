...
═══════════════════════════════════════════════════════════════════════════
[下一步开发锚点 · v1.9.0 留] 「写材料」场景（主理人明确要做的下一个方向）
当主理人下次提到「投后报告模板填充」「微信问题生成 Word 答复」「帮机构写材料」时，
本文件的 generate_answer_draft 是最近的起点。要点（详见 AGENTS.md「下一步开发方向」）：
  - 检索语料用阶段1 落库的 dd_asset_index.content_text（全文），不要只用文件名/摘要；
  - 产出的是【新答复草稿】（场景B），不替换审过的原件（场景A 仍交原件）；
  - 每条结论必须带【出处：某文件第几段】可溯源，人审后再发——这是它和"瞎编"的分界。
═══════════════════════════════════════════════════════════════════════════
"""
