...
⚠️ 拆分规则（重要）：一条里若包含多份独立材料，必须拆成多条独立需求项，便于逐份匹配：
- 含多个年度的（如"近三年财务报表""2022/2023/2024 审计报告""去年、今年、明年的预测"）
  → 按年度逐条拆开（一年一条），requirement 写清具体年度。
- 用顿号/逗号/及/和 并列多份不同材料的（如"公司章程及历次股东会决议"）→ 拆成多条。
- 同一份材料的同义表述（如"营业执照复印件"）→ 保持一条，不要重复拆。
