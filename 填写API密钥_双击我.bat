...
if %DEEP_OK%==0 (
    echo  ⚠️  必填 Key 未完成（DEEPSEEK_API_KEY），AI 评估功能将无法使用。
...
if %DASH_OK%==0 (
    echo  ⚠️  必填 Key 未完成（DASHSCOPE_API_KEY），语音转写功能将无法使用。
    echo  请重新填写，或向王波确认 Key 是否正确。
    echo.
    pause
    goto :eof
...
echo.
echo  提示：也可以在系统界面右上角点击 ⚙️ 齿轮图标，在线填写并测试 API Key。
echo.
pause
